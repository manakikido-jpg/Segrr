...
- 数量は**小数**(`Decimal(10,2)`)。0.5人日・7.5時間・0.25時間の請求に対応する。
  `数量 × 単価` が小数になる場合は**ゼロ方向に切り捨て**る(floor ではない。
  値引き行で floor すると値引き額が勝手に1円増えるため)
- **スキーマを変更したら必ず `npm run db:migrate` を使う**(`prisma migrate dev` 単体だと
  Prisma Client が再生成されず、`Decimal` 列に渡した小数が黙って切り捨てられる事故が起きる)
